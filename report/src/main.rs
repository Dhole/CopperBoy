...
pub fn main() -> Result<(), FrontError> {
    ...
    let out_path = app.value_of("out_path").map(Path::new);
    ...
    let name = app.value_of("name").unwrap_or_else(|| {
        Path::new(path)
            .iter()
            .rev()
            .nth(0)
        ...
        for _frame in 0..60 {
            ...
            if let Some(ref out_path) = out_path {
                if total_frames % screenshot_interval == 0 {
                    ...
                    for (i, framebuffer_byte) in framebuffer.iter_mut().enumerate() {
                        *framebuffer_byte = emu.core.display.frame[i] as u8;
                    }
    ...
    if let Some(out_path) = out_path {
        ...
        let file = File::create(path).unwrap();
        let w = &mut BufWriter::new(file);
        emu.core.stats.write_summary(w, 32, 32, 64).unwrap();
    }
